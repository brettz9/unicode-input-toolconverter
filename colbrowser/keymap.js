 ...
 * alphabetical order.
 *
 * Shifted punctuation characters are normally returned as the appopriate
...
Keymap.prototype.install = function(element) {
    ...
    if (element.addEventListener) {
        element.addEventListener("keydown", _handler, false);
        element.addEventListener("keypress", _handler, false);
        //element.addEventListener("keyup", handler, false); // Brett added
    }
    else if (element.attachEvent) {
        element.attachEvent("onkeydown", _handler);
        element.attachEvent("onkeypress", _handler);
        //element.attachEvent("onkeyup", handler); // Brett added
    }
    else {
        element.onkeydown = element.onkeypress = _handler;
        //element.onkeyup = handler; // Brett added
    }
};
...
Keymap.keyCodeToFunctionKey = { 
    8:"backspace", 9:"tab", 13:"returnname", 19:"pause", 27:"escape", 32:"space",
...
Keymap.prototype.dispatch = function(event) {
    ...
    var code;
    if (e.type == "keydown") { // || e.type == "keyup" // Brett added keyup
        code = e.keyCode;
        // Ignore keydown events for Shift, Ctrl, and Alt
        if (code == 16 || code == 17 || code == 18) {
            return true; // Brett added true
        }
