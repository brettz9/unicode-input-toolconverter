 ...
 */
XML.XPathExpression.prototype.getNodes = function(context) { // , xpathText, namespaces, docreplace
        /*
        if (context instanceof DocumentFragment) {
                //alert('abc'+this.xpathExpr);
//                alert('a3'+docreplace); // documentFragment
 //               alert('a4'+xpathText); //   /*
//                alert('a5'+context); // documentFragment
                docreplace = document;
                this.xpathExpr = docreplace.createExpression(xpathText,
                  // This function is passed a
                  // namespace prefix and returns the URL.
                  function(prefix) {
                          return null;
    ...
    if (this.xpathExpr) {
     ...
     XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,
        ...
        try {
            ...
            if (!this.xpathText) {return false;} // Brett added

            return context.selectNodes(this.xpathText);
        }
        catch(e) {
            // If the IE API doesn't work, we just give up
            throw "There was an error of some kind. It is possible XPath is not supported by this browser.";
        }
    }
};
...
XML.getNodes = function(context, xpathExpr, namespaces, docreplace) {
//        alert(xpathExpr);
    return new XML.XPathExpression(xpathExpr, namespaces, docreplace).getNodes(context); // , xpathExpr, namespaces, docreplace
};
// A utility to create an XML.XPathExpression and call getNode() on it 
XML.getNode = function(context, xpathExpr, namespaces, docreplace) {
    return new XML.XPathExpression(xpathExpr, namespaces, docreplace).getNode(context);
};