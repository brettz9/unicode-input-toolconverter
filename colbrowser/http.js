...
HTTP._factories = [
    function () {
        var chrome = AsYouWish.requestPrivs(['chrome']);
        Cc = chrome.Cc;
        Ci = chrome.Ci;
        var xhrPrivObj = AsYouWish.requestPrivs(['sdk/net/xhr']);
        return new xhrPrivObj.XMLHttpRequest();
    },
...
HTTP.newRequest = function() {
    ...
    return false; // Brett added this line to return some value
};
 ...
 */
HTTP.getText = function(url, callback, asynch, returnme) {
        var request = HTTP.newRequest();
        if (asynch) {
                request.onreadystatechange = function() {
                        if (request.readyState === 4 && (request.status === 200 || request.status === 0)) { // Brett added condition for status 0, local files
        ...
        // Always get a fresh copy!!! (otherwise was caching)
        
        // Fix: Use BinFileReader.js for IE support?
        request.setRequestHeader('If-Modified-Since', 'Thu, 1 Jan 1970 00:00:00 GMT');
        request.setRequestHeader('Cache-Control', 'no-cache');
        request.overrideMimeType('text/plain; charset=utf-8'); //'x-user-defined'

        request.send(null);
        if(returnme && !asynch && (request.status === 200 || request.status === 0) && request.responseText !== null) {
                return request.responseText;
        }
        if(!asynch && (request.status === 200 || request.status === 0) && request.responseText !== null) {
                callback(request.responseText);
                return true; // Brett added
        }
        else if (!asynch) {
                callback(request.status);
                return false; // Brett added
        }
 ...
 * This function does not block and has no return value.
 */
HTTP.getXML = function(url, callback, asynch) {
        var request = HTTP.newRequest();
        if (asynch) {
                request.onreadystatechange = function() {
                        if (request.readyState === 4 && (request.status === 200 || request.status === 0)) { // Brett added condition for status 0, local files
        ...
        if(!asynch && (request.status === 200 || request.status === 0) && request.responseXML !== null) {
                callback(request.responseXML);
        }
        else if (!asynch) {
               var text = HTTP.getText(url, callback, asynch, true);
                if (!text) {
                        callback(request.status);
                }
                else {
                        alert(XML.serialize(XML.parse(text)));
