...
function Tooltip() {  // The constructor function for the Tooltip class
    ...
    // Brett added/changed
        this.content = document.createElement("input"); // create div for content
    this.content.type = 'text';
    this.content.style.position = "relative";     // relatively positioned
        this.content.className = "tooltipContent";    // so we can style it
    
    var inputarea = this.tooltip;
    var inputbox = this.content;
    function _returnHandler (el, keyid, e) {
        // Save new filename and reflect change in select menu option
        var newtext = inputbox.value;
        inputarea.parentNode.removeChild(inputarea); // Remove the input box
        // How to get it back????
        Tooltip.lastel.options[Tooltip.lastel.selectedIndex].innerHTML = newtext;
        Tooltip.lastel.focus();
    }
        
        var bindings = {returnname:_returnHandler}; // Map key to handler
    var keymap = new Keymap(bindings);
    keymap.install(this.content);
...
Tooltip.prototype.show = function(text, x, y) {
    ...
    this.content.value = text;             // Set the text of the tooltip. // was innerHTML (Brett)
