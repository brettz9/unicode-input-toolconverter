...
if (typeof chrome !== 'undefined' && chrome.contextMenus) {
  ...
  chrome.runtime.onInstalled.addListener(() => {
    ...
    // Populate "Saved Items" submenu, if any exist
  ...
  const rebuildSavedItemsSubmenu = (items) => {
    ...
    chrome.contextMenus.remove('unicode-saved-items', () => {
      ...
      // Only show the submenu when there are individual saved characters
      if (!items.length) {
        return;
      }
  ...
  chrome.runtime.onMessage.addListener((
    /** @type {{action: string, item: string, items: string[]}} */ request
  ) => {
